...
const normalizeDisableCamelCase = disableCamelCase => {
  if (
    disableCamelCase &&
    typeof disableCamelCase !== 'boolean' &&
    !(disableCamelCase instanceof RegExp)
  ) {
    log.fatal("'disableCamelCase' should be a regular expression or boolean");
  }
...
module.exports = (configPath, options = {}) => {
  ...
  const rules = config.rules.map(r => ({
    ...
    disableCamelCase: normalizeDisableCamelCase(r.disableCamelCase)
  }));
  ...
  if (resolveFile && typeof resolveFile !== 'function') {
    log.fatal("'resolveFile' should be a function");
  }
